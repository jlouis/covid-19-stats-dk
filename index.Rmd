...
The other keypoint is that of the `aes(..)` function. This is an *aesthetic*-mapping.
For example, the expression `aes(x=date, y=cases)` means to map the `date` column to the
...
plot_country_lag <- function(p, title, subtitle) {
...
```{r country-weekday}
plot_day_of_week <- function(p, title, subtitle) {
...
plot_age_cases(p, "Covid-19 cases per 100k in Denmark, by age group", "Source: ssi.dk")
```

```{r age-group-tests}
p <- ggplot(weekly, aes(x=week, y=ageGroup, fill=tested_100k))
plot_age_cases(p, "Covid-19 tests per 100k in Denmark, by age group", "Source: ssi.dk")
```
